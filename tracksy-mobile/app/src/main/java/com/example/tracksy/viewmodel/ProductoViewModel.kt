...
class ProductoViewModel(
    ...
    fun cargarProductos(busqueda: String? = null) {
        viewModelScope.launch {
            ...
            try {
                ...
                val response = repo.getProductos(search = busqueda, pageSize = pageSize)
    ...
    fun cargarFavoritos() {
        viewModelScope.launch {
            try {
                val favResponse = repo.getFavoritos()
                if (favResponse.isSuccessful) {
                    ...
                    val productos = favoritosItems.mapNotNull { fav ->
                        productosEnMemoria.firstOrNull { it.id == fav.producto }
                            ?: try {
                                val r = repo.getProducto(fav.producto)
            ...
            try {
                if (esFavorito) {
                    ...
                    if (favResponse.isSuccessful) {
                        ...
                        if (favId != null) repo.removeFavorito(favId)
                    }
                }
            } catch (_: Exception) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (codigoBarras != null) {
                    val resp = repo.getProducto(codigoBarras)
                ...
                val resp = repo.getProductos(search = query)
    ...
    class Factory(private val context: Context) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T =
            ProductoViewModel(TracksyRepository()) as T
    }
}
